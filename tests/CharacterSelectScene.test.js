...
describe('CharacterSelectScene', () => {
  ...
  it('should preload assets from config', () => {
    ...
    rosterConfig.forEach(char => {
        expect(scene.load.image).toHaveBeenCalledWith(`portrait_${char.id}`, char.portraitPath);
        expect(scene.load.image).toHaveBeenCalledWith(`icon_${char.id}`, char.iconPath);
    });
  });
  ...
  it('should select character and update state', () => {
    ...
    expect(scene.leftPortrait.setTexture).toHaveBeenCalledWith(`portrait_${rosterConfig[0].id}`);
    expect(scene.nameText.setText).toHaveBeenCalledWith(rosterConfig[0].displayName.toUpperCase());
    
    // Check visual highlight
    expect(scene.gridItems[0].border.setStrokeStyle).toHaveBeenCalledWith(4, 0xffd700);
  });
