...
describe('ArenaSelectScene', () => {
  ...
  it('should fetch cities and then photos for each city', async () => {
    ...
    const mockPhotosParis = [{ url: '/cache/paris/1.webp', filename: '1.webp' }];
    const mockPhotosTokyo = [{ url: '/cache/tokyo/2.webp', filename: '2.webp' }];
    ...
    expect(global.fetch).toHaveBeenCalledWith('http://localhost:3000/api/cities');
    expect(global.fetch).toHaveBeenCalledWith('http://localhost:3000/api/photos?city=paris');
    expect(global.fetch).toHaveBeenCalledWith('http://localhost:3000/api/photos?city=tokyo');
    
    expect(scene.arenas).toHaveLength(2);
    expect(scene.arenas[0]).toEqual({ name: 'paris', url: 'http://localhost:3000/cache/paris/1.webp' });
  });
  ...
  it('should update selected arena state', () => {
    ...
    expect(scene.thumbnails[1].border.setStrokeStyle).toHaveBeenCalledWith(4, 0xffd700);
  });
});
